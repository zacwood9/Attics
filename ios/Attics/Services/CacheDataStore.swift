...
//    }
//
//    func fetchTopShows(collection: String, numShows: Int, then completion: @escaping (Result<[YearWithTopShows]>) -> ()) {
//        let fr = YearMO.fetchRequest() as NSFetchRequest<YearMO>
...
//            let shows = showMOs.map(Show.init).sorted { (Double($0.numReviews)*$0.avgRating) > (Double($1.numReviews)*$1.avgRating) }
